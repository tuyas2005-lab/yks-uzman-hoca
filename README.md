...
- Telefon/tablet için Supabase tabanlı V5 senkronizasyonu: `student_state` + append-only `student_event_revisions`
...
- Supabase senkronizasyonu için `SUPABASE_URL` ve `SUPABASE_PUBLISHABLE_KEY`

API anahtarı istemci koduna yazılmaz. OpenAI Responses API çağrılarında `store:false` kullanılır.

## Veri ve senkronizasyon
Yerel çalışma verisi tarayıcıda saklanır. StudyEvent V5, Mini Test, Soru Çöz, Kişisel Öğretmen ve deneme verilerini ortak modele taşır. Çok cihazlı Supabase senkronizasyonu `student_state` tablosunda genel durumu, `student_event_revisions` tablosunda ise StudyEvent revizyonlarını append-only olarak saklar. Her iki tablo RLS ile kullanıcı kimliğine sınırlandırılır; event ledger istemciler için güncellenemez ve silinemez.
