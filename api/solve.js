  ...
  try{
    ...
    try{
      ...
      verificationResponse=await client.responses.create({
        model:ECONOMY_MODEL,store:false,reasoning:{effort:"none"},max_output_tokens:440,
        input:[
          {role:"developer",content:`YKS cevap doğrulayıcısısın. İlk modelin sonucunu onaylamak için değil, soruyu bağımsız yeniden çözmek için çalışırsın.\n1. Önce soru metnini ve varsa görseli aday cevaptan bağımsız çöz.\n2. Matematikte mümkünse aritmetik, denklem çözme, oran-yüzde hesabı, seçenek yerine koyma veya sonuç geri-kontrolü yap.\n3. Sonra aday answer doğru mu ve short_solution aynı cevabı gerçekten destekliyor mu kontrol et.\n4. Doğrudan çelişkide consistent=false olmalı. Örneğin answer E iken açıklama E seçeneğini yanlışlıyorsa tutarlı sayma.\n5. verifiedAnswer gerçek final cevap; verifiedShortSolution onu destekleyen en fazla 2-3 kısa cümle olsun.\n6. Gerçekten doğrulayamıyorsan confidence=low kullan; tahmin yürütme.\n7. reason yalnız kısa doğrulama özeti olsun; gizli düşünme zinciri yazma.`},
    ...
    if(!candidateIsConsistent(candidate,verification)&&verification.confidence!=="low"){
      ...
      try{
        const correctionInput=questionContent(text,image);
        ...
        correctionResponse=await client.responses.create({
          model:ECONOMY_MODEL,store:false,reasoning:{effort:"none"},max_output_tokens:360,
          input:[
            {role:"developer",content:`YKS sorusu için düzeltilmiş final çözümü üret. Soruyu tekrar kontrol et; güvenilmeyen ilk cevabı kopyalama. Bağımsız verifier sonucunu yalnız soruyla uyumluysa kullan. answer ile short_solution mutlaka aynı sonucu savunmalı. short_solution en fazla 2-3 kısa cümle olsun. Cevap güvenilir biçimde doğrulanamıyorsa answer alanına ${JSON.stringify(UNVERIFIED_ANSWER)}, short_solution alanına ${JSON.stringify(UNVERIFIED_SOLUTION)} yaz.`},
