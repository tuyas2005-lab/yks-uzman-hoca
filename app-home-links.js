(()=>{
  ...
  if(typeof go==='function'&&!window.__connectedGo){const base=go;go=function(id){try{window.YKSDataV5?.syncLegacy?.();window.refreshLearningModel?.()}catch{}const r=base(id);if(id==='home')setTimeout(()=>window.renderHome?.(),0);if(id==='topics')setTimeout(()=>window.renderTopics?.(),0);if(id==='wrong')setTimeout(()=>window.renderWrong?.(),0);if(id==='stats')setTimeout(()=>window.renderStats?.(),0);if(id==='tests')setTimeout(()=>applyMiniPrefill(),0);if(id==='teacher')setTimeout(()=>{try{window.renderTeacher?.()}catch{}},0);if(id==='coach')setTimeout(()=>{try{window.renderCoach?.()}catch{}},0);return r};window.go=go;window.__connectedGo=true}
})();
