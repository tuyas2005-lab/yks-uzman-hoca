(()=>{
  ...
  const growthLabels=['Başlangıç','Temel','Gelişiyor','Güçlü','Kalıcı'];
  function growthView(f){
    const P=window.YKSTeacherPilotV1,topicId=P?.resolveTopic?.(f)?.id,evidence=P?.buildTopicProgressEvidence?.(state.studyEvents||[],{topicId,today:today()})||{},progress=P?.assessTopicProgress?.(evidence)||{level:1,stage:'starting'},level=Math.min(5,Math.max(1,Number(progress.level||1)));
    const messages=[
      'Seni gerçek sorularla tanıyorum. İlk ölçümlerden sonra yolunu daha net çizeceğim.',
      'Temeli birlikte kuruyoruz. Açık yanlışlarını kapattıkça ilerlemen hızlanacak.',
      'Gelişiyorsun. Aynı başarıyı farklı bir çalışma gününde de gördüğümde sıradaki seviyeye geçeceğiz.',
      'Bu konuda güçlü bir seviyeye ulaştın. Yeni konuya ilerlerken bunu kısa kontrollerle koruyacağız.',
      'Bu bilgiyi uzun süre koruduğunu kanıtladın. Artık yalnız ihtiyaç olduğunda kısa kontrol yapacağız.'
    ];
    const steps=growthLabels.map((label,i)=>{const n=i+1,cls=n<level?'past':n===level?'current':'future';return`<div class="pt2-growth-step ${cls}"><span>${n<level?'✓':n}</span><small>${esc(label)}</small></div>`}).join('');
    return`<section class="pt2-growth" aria-label="${esc(f.topic)} konu gelişim seviyesi ${level}/5"><div class="pt2-growth-head"><div><small>BU KONUDAKİ YOLCULUĞUN</small><h3>${esc(growthLabels[level-1])}</h3></div><b>${level}/5</b></div><div class="pt2-growth-steps">${steps}</div><p><b>Öğretmenin:</b> ${esc(messages[level-1])}</p></section>`;
  }
  function sourceHealthView(){
    const P=window.YKSTeacherPilotV1,report=P?.buildSourceHealthReport?.(C()?.all?.()||[])||[],candidates=report.filter(x=>!x.healthy).sort((a,b)=>{const ad=Object.values(a.deficits||{}).reduce((s,n)=>s+n,0),bd=Object.values(b.deficits||{}).reduce((s,n)=>s+n,0);return ad-bd||b.ready-a.ready}).slice(0,4);
    if(!candidates.length)return'';
    const rows=candidates.map(x=>{const deficits=Object.entries(x.deficits||{}).filter(([,n])=>n>0).map(([d,n])=>`${d.toLocaleLowerCase('tr-TR')} ${n} eksik`).join(' • '),status=x.canonicalTopicId?'Kaynak tamamlanmayı bekliyor':'Konu eşleşmesi inceleniyor';return`<div class="pt2-source-row"><div><b>${esc(x.sourceTitle)}</b><small>${esc(status)} • ${x.ready} hazır${x.pending?` • ${x.pending} bekliyor`:''}</small></div><span>${esc(deficits||'hazırlanıyor')}</span></div>`}).join('');
    return`<section class="pt2-source-health" aria-label="Kaynak sağlığı ve sıradaki konular"><div class="pt2-source-head"><div><small>KAYNAK SAĞLIĞI</small><h3>Öğretmenin sıradaki konu adayları</h3></div><span>15 / seviye</span></div><p>Bir konuyu sana açmadan önce kolay, orta ve zor sorulardan yeterli sayıda hazır olması gerekiyor.</p><div class="pt2-source-list">${rows}</div></section>`;
  }
