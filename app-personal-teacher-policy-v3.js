(()=>{
  ...
  function todayQuestions(){return (state.studyEvents||[]).filter(x=>x?.dateKey===today()).reduce((a,x)=>a+Number(x.questionCount||0),0)}
  ...
  function render(){
    const f=focus(),d=ensureDaily(f),info=modeInfo[d.mode]||modeInfo.diagnostic,count=usableCount(f,d),perf=recent(f),macro=macroFor(f),tasks=stepsHtml(f,d,count),total=(tasks.match(/class="pt2-task/g)||[]).length,done=(tasks.match(/class="pt2-task done/g)||[]).length,goal=Number(state.profile?.goal||10),todayQ=todayQuestions();
    root.innerHTML=`<div class="screen-head"><button class="back" data-go="home">←</button><h1>Kişisel Öğretmenim</h1></div><div class="pt2"><div class="pt2-hero"><div><div class="pt2-chips"><span class="pt2-chip">${esc(`${f.exam} ${f.subject}`)}</span><span class="pt2-chip green">${esc(info.label)}</span>${macro?`<span class="pt2-chip">Koç payı %${macro.share||0}</span>`:''}<span class="pt2-chip">${esc(f.confidence||'Veri az')}</span></div><h2>${esc(f.topic)}</h2><p><b>${esc(info.title)}.</b> ${esc(reason(f,d))}</p></div><div class="pt2-progress"><b>${done}/${total}</b><small>bugünkü plan</small><div class="pt2-bar"><i style="width:${total?Math.round(done/total*100):100}%"></i></div></div></div><div class="pt2-main"><div class="pt2-card"><div class="pt2-head"><h3>Bugünkü Akış</h3><small>Veriye göre sıra ve yük değişir</small></div><div class="pt2-tasks">${tasks}</div><div id="pt2RecapSlot" style="margin-top:12px"></div></div><div class="pt2-card"><div class="pt2-head"><h3>Karar Kanıtları</h3><small>V5</small></div><div class="pt2-metrics"><div class="pt2-metric"><b>${f.score==null?'—':`%${f.score}`}</b><small>ölçümlü başarı</small></div><div class="pt2-metric"><b>${Number(f.recentWrong||0)}</b><small>yakın yanlış</small></div><div class="pt2-metric"><b>${count}</b><small>şimdi kullanılabilir kaynak</small></div><div class="pt2-metric"><b>${todayQ}/${goal}</b><small>bugünkü soru yükü</small></div></div><div class="pt2-note">Karar sırası: alan → Koç/deneme önceliği → konu ölçümü → son yanlışlar ve zorlanma → veri güveni → tekrar aralığı → bugünkü yük. AI soru üretmez.</div></div></div><div class="pt2-details"><details><summary>Öğretmenin konu seçimini değiştir</summary><div class="pt2-topics">${candidates().slice(0,5).map(x=>`<button class="pt2-topic ${norm(x.topic)===norm(f.topic)?'active':''}" data-pt3-topic="${esc(x.topic)}"><span><b>${esc(x.topic)}</b><br><small class="muted">${esc(`${x.exam} ${x.subject}`)}</small></span><span>${x.score==null?'—':`%${x.score}`}</span></button>`).join('')}</div></details><details><summary>Son performansı göster</summary><div class="pt2-perfs">${perf.length?perf.map(x=>`<span class="pt2-perf ${x.result==='correct'?'ok':'bad'}">${x.result==='correct'?'✓':'✕'} ${new Date(x.timestamp||Date.now()).toLocaleDateString('tr-TR',{day:'2-digit',month:'2-digit'})}</span>`).join(''):'<span class="muted">Henüz ölçüm yok.</span>'}</div></details></div></div>`;
