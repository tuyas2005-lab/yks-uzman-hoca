(()=>{
  ...
  function decorate(){
    for(const x of C()?.all?.()||[]){
      ...
      const old=x.asset||{},pdfKey=pdfKeyFor(x);
      x.asset={status:'ready',kind:'auto-text-crop',pdfKey,page:Number(x.access.page),crop:old.crop};
  ...
  function install(tries=0){
    ...
    window.prepareSourceQuestions=async function(items=[]){
      const list=(items||[]).filter(Boolean),auto=list.filter(x=>eligible(x)&&!hasManual(x));
