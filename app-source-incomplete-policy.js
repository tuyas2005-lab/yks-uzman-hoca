(()=>{
  ...
  function itemState(item){
    ...
    const answerReady=!!(item?.answerKey&&item?.answerVerified);
  ...
  function showIncomplete(item,ctx={}){
    ...
    root.innerHTML=`<div class="sq-meta"><span class="sq-chip">${esc(item.providerLabel||item.provider||'Kaynak')}</span><span class="sq-chip">${esc(`${item.year||''} ${item.exam||''}`.trim())}</span><span class="sq-chip">${esc(`${item.subject||''} • ${item.topic||''}`)}</span><span class="sq-chip sq-pending">Eksik kayıt</span></div><div class="sq-card"><div class="sq-image"><div class="sq-loading"><b>Bu soru kütüphanede mevcut ancak henüz tamamen hazırlanmadı.</b><br><br>Eksik: ${esc(missing.join(' + ')||'soru içeriği')}<br><small>Soru havuzdan çıkarılmadı. Kütüphaneyi beslerken bu kaydı tamamlayacağız.</small></div></div></div><div class="sq-card"><b>${esc(item.collection||'Kaynak')}</b><p class="muted">${esc(item.subject||'')} • ${esc(item.topic||'')} • Soru ${esc(item.questionNo||'—')}</p><button id="sqIncompleteBack" class="ghost">${ctx?.type==='mini'?'Sete Dön':ctx?.type==='wrong'?'Yanlışlarıma Dön':'Geri Dön'}</button></div>`;
  ...
  function install(tries=0){
    ...
    c.selectionPolicy={...(c.selectionPolicy||{}),mode:'library-only-indexed-first',allowIncomplete:true,readyRequired:false,yearGate:true};
    ...
    window.isSourceQuestionReady=item=>!!item?.id;
