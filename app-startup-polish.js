(()=>{
  ...
  const html=document.documentElement;let finished=false;
  function loadVisualIdentity(){
    ...
    s.onload=()=>{if(document.querySelector('script[data-visual-identity-v2]'))return;const x=document.createElement('script');x.src='/app-visual-identity-v2.js?v=2';x.async=true;x.dataset.visualIdentityV2='true';x.onload=()=>{if(!document.querySelector('script[data-visual-identity-v3]')){const y=document.createElement('script');y.src='/app-visual-identity-v3.js?v=2';y.async=true;y.dataset.visualIdentityV3='true';document.body.appendChild(y)}};document.body.appendChild(x)};
    s.onerror=()=>console.warn('Yeni görsel kimlik yüklenemedi.');document.body.appendChild(s);
  }
  function finish(reason='ready'){if(finished)return;finished=true;try{window.renderHome?.();window.renderTopics?.()}catch(e){console.warn('Başlangıç son render:',e)}html.classList.remove('yks-startup-error');html.classList.add('yks-ready');html.dataset.bootReason=reason;loadVisualIdentity()}
