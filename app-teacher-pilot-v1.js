(()=>{
  ...
  function resumeMode(raw={}){const previous=String(raw.previousMode||''),next=String(raw.nextMode||''),today=String(raw.today||''),reviewDate=String(raw.nextReviewDate||'');if(raw.closureComplete===false)return'repair';if(['diagnostic','foundation','repair','reinforce'].includes(previous)&&MODE_CONFIG[next])return next;if(reviewDate&&today&&today>=reviewDate)return'spaced';if(['challenge','spaced','maintain'].includes(previous))return'maintain';return MODE_CONFIG[next]?next:''}
