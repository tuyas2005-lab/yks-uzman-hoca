(()=>{
  ...
  style.textContent=`
    ...
    .tws-list{display:grid;gap:9px}.tws-item{display:grid;grid-template-columns:40px minmax(0,1fr) auto;gap:11px;align-items:center;border:1px solid var(--line);border-radius:14px;padding:12px;background:var(--surface)}.tws-item.done{border-color:#bce3ca;background:#f8fdf9}.tws-no{width:32px;height:32px;border-radius:10px;background:#eeeaff;color:#5942d1;display:grid;place-items:center;font-weight:900}.tws-item.done .tws-no{background:#def4e6;color:#187649}.tws-item b{display:block;font-size:13px}.tws-item small{display:block;color:var(--muted);font-size:11px;margin-top:3px}.tws-open{border:0;border-radius:10px;padding:9px 11px;background:#6747eb;color:#fff;font-weight:850}.tws-item.done .tws-open{background:#edf2ef;color:#557064}.tws-actions{display:flex;justify-content:flex-end;margin-top:12px}.tws-empty{padding:18px;border:1px dashed var(--line);border-radius:14px;color:var(--muted);text-align:center}@media(max-width:700px){.tws-head{flex-direction:column}.tws-item{grid-template-columns:36px 1fr}.tws-open{grid-column:1/-1;width:100%}}
  `;
  ...
  function wrongRows(scope){
    let rows=(state.studyEvents||[]).filter(x=>x?.meta?.wrongRecord===true);
