(()=>{
  ...
  function recordReview(id){
    ...
    const reviewedAt=Date.now();try{window.YKSDataV5?.record?.({source:'wrong-review',exam:src.exam,track:src.track,subject:src.subject,topic:src.topic,curriculumOutcome:src.curriculumOutcome||src.meta?.solution?.curriculumOutcome||'',result:'unknown',difficulty:src.difficulty,interaction:'reviewed',questionCount:0,signals:['reviewed-wrong'],meta:{reviewOf:id,reviewedAt,dailyPriority:true}},{persistNow:true});window.markWrongLearningEvidence?.(id,{wrongReviewedAt:reviewedAt})}catch(e){console.warn('Yanlış tekrar kaydı oluşturulamadı',e)}
