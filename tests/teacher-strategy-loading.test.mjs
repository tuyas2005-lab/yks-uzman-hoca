...
test('Startup and service worker use the new loader cache key', () => {
  assert.match(startup, /\/app-home-links\.js\?v=21/);
  assert.doesNotMatch(startup, /\/app-home-links\.js\?v=20/);
  assert.match(serviceWorker, /\/app-home-links\.js\?v=21/);
  ...
  assert.match(serviceWorker, /r47-longitudinal-pilot/);
});
