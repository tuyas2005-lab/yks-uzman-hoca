...
test('Teacher loads the shared Strategy Engine before policy v3', () => {
  ...
  const policy = teacher.indexOf("loadScript('/app-personal-teacher-policy-v3.js?v=11')");
...
test('Startup and service worker use the new loader cache key', () => {
  assert.match(startup, /\/app-home-links\.js\?v=20/);
  assert.doesNotMatch(startup, /\/app-home-links\.js\?v=19/);
  assert.match(serviceWorker, /\/app-home-links\.js\?v=20/);
  assert.match(startup, /\/app-teacher-pilot-v1\.js\?v=8/);
  assert.match(serviceWorker, /\/app-teacher-pilot-v1\.js\?v=8/);
  assert.match(serviceWorker, /\/app-personal-teacher-source-launch-v3\.js\?v=8/);
  assert.match(serviceWorker, /r46-teacher-audit/);
});
