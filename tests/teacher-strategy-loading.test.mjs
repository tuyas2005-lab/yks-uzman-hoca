...
test('Startup and service worker use the new loader cache key', () => {
  assert.match(startup, /\/app-home-links\.js\?v=31/);
  assert.doesNotMatch(startup, /\/app-home-links\.js\?v=30/);
  assert.match(serviceWorker, /\/app-home-links\.js\?v=31/);
  ...
  assert.match(serviceWorker, /r57-human-teacher-voice/);
});
