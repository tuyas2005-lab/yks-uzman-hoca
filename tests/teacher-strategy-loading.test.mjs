...
test('Teacher loads the shared Strategy Engine before policy v3', () => {
  ...
  const policy = teacher.indexOf("loadScript('/app-personal-teacher-policy-v3.js?v=4')");
...
test('Startup and service worker use the new loader cache key', () => {
  assert.match(startup, /\/app-home-links\.js\?v=16/);
  assert.doesNotMatch(startup, /\/app-home-links\.js\?v=15/);
  assert.match(serviceWorker, /\/app-home-links\.js\?v=16/);
  assert.match(serviceWorker, /r30-teacher-learning-loop/);
});
